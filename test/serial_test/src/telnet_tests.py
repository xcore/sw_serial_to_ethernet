...
class XmosTelnetTestSuite(XmosTelnetTest):    
    ...
    def application_telnet_read_write_command_check(self, channel_count=8):
        ...
        try:
            ...
            self.print_to_log(test_name, '                 GET and SET')
            self.print_to_log(test_name, '*******************************************')
            for chan_id in range(0,channel_count):
                stage = 0
                try:
                    config_str = '~2~~'+str(chan_id)+'~~2~~0~~115200~~8~~'+str(46+chan_id)+'~@'
                    self.print_to_log(test_name, '----CHECKING TELNET SET-----')
                    self.print_to_log(test_name, 'For Uart '+str(chan_id)+' Telnet Port '+str(46+chan_id))
                    self.print_to_log(test_name, 'Command Sent:'+config_str)
                    telnet_session.sendline(config_str)
                    telnet_session.expect(re.escape(config_str), timeout=5)
                    self.print_to_log(test_name, 'Response Received:'+config_str)
                    self.print_to_log(test_name, "-----CHECKING TELNET GET-----")
                    telnet_session.sendline('~1~~'+str(chan_id)+'~@')
                    self.print_to_log(test_name, 'Command Sent : '+' ~1~~'+str(chan_id)+'~@')
                    ...
                    break
            self.print_to_log(test_name, '\n***************************************************')
            self.print_to_log(test_name, '                 SAVE and RESTORE')
            self.print_to_log(test_name, '***************************************************')
            for chan_id in range(0,channel_count):
                stage = 0
                try:
                    config_str = '~2~~'+str(chan_id)+'~~2~~0~~9600~~8~~'+str(46+chan_id)+'~@'
                    self.print_to_log(test_name, '----CHECKING TELNET SAVE(SET) COMMAND-----')
                    self.print_to_log(test_name, 'For Uart '+str(chan_id)+' Telnet Port '+str(46+chan_id))
                    self.print_to_log(test_name, 'Command Sent:'+config_str)
                    telnet_session.sendline(config_str)
                    telnet_session.expect(re.escape(config_str), timeout=5)
            ...
            for chan_id in range(0,channel_count):
                try:
                    ...
                    telnet_session.expect(re.escape(config_str),timeout=5)
                    self.print_to_log(test_name, 'Response : '+ config_str)
